...
def _route_dict(o, extended=False):
    ret = {
        ...
        'name': o.name,
        'direction': {
            'id': o.direction.id, 'name': o.direction.name
        },
        'start_station': {
            'id': o.start_station.id, 'name': o.start_station.name
        },
        'end_station': {
            'id': o.end_station.id, 'name': o.end_station.name
        },
    }
    if extended:
        traveltime = '-'
        if o.routestation_set.count():
            traveltime = str(
                o.routestation_set.aggregate(
                    django.db.models.Sum('move_time')
                )['move_time__sum'] +
                o.routestation_set.aggregate(
                    django.db.models.Sum('wait_time')
                )['wait_time__sum']
            )
...
class SearchAPI(API):
    ...
    def search(cls, request):
        ...
        if error:
            ...
            ret['start_station'] = _station_dict(
                trains.models.Station.objects.get(pk=start_station))
            ret['end_station'] = _station_dict(
                trains.models.Station.objects.get(pk=end_station))
            ret['date'] = date
            ...
            for obj in routes:
                rr.append({
                    ...
                    'routes': [{
                            'route': _route_dict(o['route']),
                            'time': str(o['time'])} for o in obj['routes']]
                })
...
class ViewAPI(API):
    ...
    def station(cls, request, pk):
        ...
        ret['directionstation'] = [
            _directionstation_dict(ds)
            for ds in o.directionstation_set.all()
        ]
        ret['routesstation'] = [
            _routestation_dict(rs)
            for rs in o.routestation_set.all()
        ]
        return cls._response(json.dumps(ret))
    ...
    def stations_list(cls, request):
        ret = [
            _station_dict(s, extended=True)
            for s in trains.models.Station.objects.all()
        ]
        return cls._response(json.dumps(ret))
    ...
    def direction(cls, request, pk):
        ...
        ret['route'] = [
            _route_dict(r, extended=True)
            for r in o.route_set.all()
        ]
        return cls._response(json.dumps(ret))
    ...
    def directions_list(cls, request):
        ret = [
            _direction_dict(d, extended=True)
            for d in trains.models.Direction.objects.all()
        ]
        return cls._response(json.dumps(ret))
    ...
    def routes_list(cls, request):
        ret = [
            _route_dict(r, extended=True)
            for r in trains.models.Route.objects.all()
        ]
        return cls._response(json.dumps(ret))
